...
def get_data_from_csv(columns, countries=None, start=None, end=None):
	...
	if countries is None:
		...
		print(df_latest_dates.nlargest(6,"new_cases_per_million"))
		countries = df_latest_dates.nlargest(6,"new_cases_per_million")

	# now filter to include only the selected countries
	cases_df = df[df["location"].isin(countries["location"])]
...
def plot_reported_cases_per_million(countries=None, start=None, end=None):
	...
	chart = (
		alt.Chart(cases_df, title="Cases")
		.mark_line()
		.encode(
			x=alt.X(
				"date:T",
